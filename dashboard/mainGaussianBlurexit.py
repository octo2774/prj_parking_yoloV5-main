...
cap = cv2.VideoCapture("B3-ParkingEXIT.avi")
...
#object_detector = cv2.createBackgroundSubtractorMOG2(history=2000, varThreshold = 1000, detectShadows=False)
...
while True:
    ...
    roi = imutils.resize(roi, width=500, height=500)
    mask = imutils.resize(mask, width=500, height=500)
    cv2.imshow("roi", roi)
